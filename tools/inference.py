...
def get_args():
    ...
    parser.add_argument(
        '--save_vis_img',
        action='store_true',
        default=True, # 原来是False
        help='whether to save img of complete point cloud') 
    parser.add_argument(
        '--out_pc_root',
        type=str,
        default='inference_result/example_GRCNet_CD_CG512_val/',
...
def inference_single(model, pc_path, args, config, root=None):
    ...
    if args.out_pc_root != '':
        ...
        # Convert dense_points to point cloud format and save as ASCII ply file
        pcd = o3d.geometry.PointCloud()
        pcd.points = o3d.utility.Vector3dVector(dense_points)
        o3d.io.write_point_cloud(os.path.join(target_path, 'fine-GRFormer.ply'), pcd, write_ascii=True)
